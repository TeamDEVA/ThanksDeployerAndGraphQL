...
class thanksEthereumClass {
  ...
  constructor() {
    let noncePath = __dirname + '/../API/nonce.txt';
  ...
  async initialize() {
    await this.mainnetHandler.initializeNonceMainnet("testnet", this.noncePath);
  }

  async findMentions(targetEntity, targetField, query, data) {
    var my_string = query;
  ...
  async write(functionName, information) {
    ...
    if (errorLog != "Success") {
      ...
      var nonce = parseInt(fs.readFileSync(this.noncePath, 'utf8'));
      ...
      if (errorLog != "Success") {
        ...
        fs.writeFileSync(this.noncePath, (nonce + 1).toString());
